...
listen to me rant about the work presented in this dissertation.
...
been able to count on the people closest to me. Each of these people has
supported me not only during the easy times, but, more importantly,
through the challenging ones. Addy, Boe, Charles, Mari, Paul, Ricky,
Spencer, Wyatt, Zane, and 보미: thank you for being such a strong and
constant support network. I'm extraordinarily lucky to have you all in
my life.
...
productivity, then what is it driven by? Is it driven by the frequency
of the phrase? Do other usage-based factors, such as predictability,
also drive storage? Additionally, if an item that can be generated as a
combination of its parts is stored holistically then in what
...
multi-word phrase when they could simply generate it by combining each
of its parts individually. We show that not only frequent phrases are
stored, but also predictable phrases. However, positing that multi-word
phrases are stored holistically creates new challenges for theories of
processing, which now must explain how multi-word phrases are
represented and accessed. Thus, we also explore how holistically stored
phrases are represented and processed. Further, we demonstrate that
large language models also trade off between stored and generative
knowledge in ways that are both similar and different from humans.
Finally, we show that by positing that multi-word phrases are stored
holistically, we can explain some aspects of language change.
...
multi-word phrases being stored holistically (e.g.,
\citeproc{ref-bybee2003}{Bybee, 2003}; \citeproc{ref-bybee2001}{Bybee \&
Hopper, 2001}; \citeproc{ref-goldbergConstructionGrammar2010}{A.
Goldberg \& Suttle, 2010}; \citeproc{ref-kapatsinski2018}{Kapatsinski,
2018}; \citeproc{ref-morgan2015}{Morgan \& Levy, 2015},
\citeproc{ref-morganAbstractKnowledgeDirect2016}{2016a},
\citeproc{ref-morgan2024}{2024};
\citeproc{ref-stembergerFrequencyLexicalStorage1986}{Stemberger \&
MacWhinney, 1986},
...
representations with no phonetic details (e.g.,
...
(\citeproc{ref-bybeeEffectUsageDegrees1999}{1999}) demonstrated that
...
(\citeproc{ref-bybeeWordFrequencyContext2002}{2002}) further argued that
...
this led to many of the same questions being asked about higher levels
...
it is possible that multi-morphemic words or even phrases may similarly
...
\citeproc{ref-stembergerFrequencyLexicalStorage1986}{Stemberger \&
MacWhinney, 1986},
\citeproc{ref-stembergerAreInflectedForms2004}{2004}). For example, by
examining corpus data, Stemberger \& MacWhinney
(\citeproc{ref-stembergerFrequencyLexicalStorage1986}{1986})
demonstrated that errors occur less in high-frequency words than in
...
2015}). For example, there is evidence that attending to the whole
utterance as opposed to attending to each individual word facilitates
learning (\citeproc{ref-siegelmanAdvantageStartingBig2015}{Siegelman \&
Arnon, 2015}). Specifically, Siegelman \& Arnon
...
\citeproc{ref-mcclellandTRACEModelSpeech1984}{McClelland et al., 1984a})
...
that could produce fluent text that was indiscernible from text written
by humans still seemed like an idea out of a Sci-Fi movie. However, with
...
sentence (\citeproc{ref-vaswaniAttentionAllYou2017}{Vaswani et al.,
...
\citeproc{ref-rumelhart1986learningtensesenglish}{Rumelhart \&
McClelland, 1986}). For example, Rumelhart \& McClelland
(\citeproc{ref-rumelhart1986learningtensesenglish}{1986}) demonstrated
...
to.\footnote{It is important to note, especially given the topic of this
  ...
  (\citeproc{ref-zeiler2014visualizing}{Zeiler \& Fergus, 2014}).}
...
Piantadosi \& Hill
(\citeproc{ref-piantadosiMeaningReferenceLarge2022}{2022}) pointed out
...
Learning theories (e.g., \citeproc{ref-kapatsinski2018}{Kapatsinski,
2018}, \citeproc{ref-kapatsinskiDefragmentingLearning2023}{2023};
...
Crucially they systematically removed the length and animacy bias from
the training data. They found that while the effect weakens, there is
still an effect of length. They argued that this is evidence that
language models are learning general patterns of the language. These
results taken together with previous results demonstrate the ability of
...
generate everything else (e.g., \citeproc{ref-pinker1991rules}{Pinker,
1991}). According to these theories, inflected words, such as
...
frequency (\citeproc{ref-arnonMoreWordsFrequency2010}{Arnon \& Snider,
2010}; \citeproc{ref-baayenAmorphousModelMorphological2011}{R. H. Baayen
et al., 2011}; \citeproc{ref-bybee2003}{Bybee, 2003};
\citeproc{ref-bybee2001}{Bybee \& Hopper, 2001};
\citeproc{ref-goldbergConstructionsNewTheoretical2003}{A. E. Goldberg,
2003}; \citeproc{ref-morgan2015}{Morgan \& Levy, 2015},
\citeproc{ref-morganAbstractKnowledgeDirect2016}{2016a},
\citeproc{ref-morgan2024}{2024};
\citeproc{ref-odonnellProductivityReuseLanguage2016}{O'Donnell, 2016};
\citeproc{ref-tomaselloConstructingLanguageUsagebased2005}{Tomasello,
...
(\citeproc{ref-bybee2003}{Bybee, 2003}). This is also not a feature of
...
on the data. They tested each of these models on a held-out test set.
They found that the Inference-based model performed the best overall for
both past tenses and derivational morphology. In other words, storing
units of varying sizes (as opposed to just minimal or maximal-sized
units) seems to be the most conducive approach to learning the various
morphological paradigms in English.
...
frequency respectively (where phrasal frequency is the total number of
times the compound noun occurred). Participants read sentences which
contained a novel compound noun or a familiar compound noun (See Example
...
might expect that readers would be able to access the holistic
representation of the entire compound noun when reading the first noun
(because the first noun is a cue to the holistic representation), thus
eliminating the local implausibility. Thus, if these items are stored
holistically it begs the question of what the processing consequences of
storage are. Alternatively, it may just be that these items are not
stored. For example, it is possible that, as has been previewed
throughout the introduction, phrasal frequency may not be the driving
factor of storage. Instead, it may actually be predictability that
drives storage. If this is the case, then it is possible that the reason
for a lack of an interaction effect in Staub et al.
...
(from \citeproc{ref-staubTimeCoursePlausibility2007}{Staub et al.,
2007}) which varied upon two dimensions: local plausibility and
...
(\citeproc{ref-boyceMazeMadeEasy2020}{2020}) using the Gulordava model
(\citeproc{ref-gulordava2018colorlessgreenrecurrent}{Gulordava et al.,
2018}). The locations of the distractor word and target word were
...
2022}).\footnote{For more details about the analyses, our materials are
...
design, the implausible sentence through the second noun (3d) is
technically plausible because the second noun eliminates the local
implausibility. Thus this condition should also receive a high rating,
despite being labeled as implausible. The mean values for each condition
are as follows: plausible, through the first noun: 5.58 (sd = 0.78);
plausible, through the second noun: 5.41 (sd = 0.71); implausible,
through the first noun: 3.13 (0.63); implausible, through the second
noun: 5.47 (sd = 0.82).
...
parameter
(\citeproc{ref-wagenmakers2010bayesianhypothesistesting}{Wagenmakers et
al., 2010}). We computed the Bayes factor for the N1 region and not the
...
Sentences were presented on a screen that was 531.3mm in width and
298.8mm in height, placed 850mm away from where the participants were
seated. The screen had a resolution of 1920x1080. For each trial, we
recorded participants' right pupil movements using the Eyelink 1000
Plus.
...
Participants were given 3 practice trials prior to the start of the
experiment. Experimental and filler trials were presented randomly and
the eye-tracker was re-calibrated every 35 trials to ensure proper
calibration.
...
Figure~\ref{fig-fullmodelresultsn2staub}. At the N2 region, we find
main-effects of familiarity for first fixation duration and gaze times,
and marginal effects for go-past times and first-pass regression. We
find no main-effect of plausibility, which is expected because the
implausible condition is plausible at the N2 region. We also find no
interaction effect between plausibility and familiarity.
...
We do find an interaction effect in the opposite direction from what we
predicted for go-past times, however since we find it in only one
eye-tracking measure it is unlikely that it indicates that the local
...
\citeproc{ref-juhasz2006binocularcoordinationeyes}{Juhasz et al.,
2006}). For example, in the below sentences, \emph{satchel} is
...
low-predictability items. Thus overall it seems that either participants
are not accessing the holistic representation at the first noun or our
assumptions about how representations map to processing is inacurrate.
...
effect. In other words, the assumptions regarding the mapping between
storage and processing measures may be incorrect.
...
compound noun to rule out other competing possibilities. Finally, the
present results may also suggest that the current assumptions regarding
the mapping between linguistic representations and processing may be
incorrect. It is possible that the mapping between representations and
processing is more complex than previously thought.
...
quite some time (e.g., \citeproc{ref-bybee2003}{Bybee, 2003};
\citeproc{ref-bybee2001}{Bybee \& Hopper, 2001};
\citeproc{ref-goldbergConstructionsNewTheoretical2003}{A. E. Goldberg,
2003}; \citeproc{ref-nooteboomStorageComputationLanguage2002}{Nooteboom
et al., 2002};
\citeproc{ref-stembergerFrequencyLexicalStorage1986}{Stemberger \&
MacWhinney, 1986},
\citeproc{ref-stembergerAreInflectedForms2004}{2004}).
...
\citeproc{ref-pinker1991rules}{Pinker, 1991};
...
2009}). One such study that did examine the role of predictability in
holistic storage was the series of experiments in Chapter 2 of this
dissertation. To refresh the reader's memory, in the previous chapter we
...
suggested that the first noun in the compound nouns was read more slowly
in the implausible condition than in the plausible condition.
...
item was checked by myself and another graduate student in the
department. Disagreement was easily resolved by discussion and an
agreement was reached for every item.
...
It is also possible that despite a lack of an interaction effect,
...
and predictability (Equation~\ref{eq-gamminteraction}).}
...
interaction between them (Equation~\ref{eq-gammFull}).}
...
(Equation~\ref{eq-BayesianFullModelSyntax}).}
...
regression model containing only predictability and
predictability\(^2\)(Equation~\ref{eq-brmsPredic}).}
...
predictability and frequency of the GAM model containing only the
interaction between frequency and predictability
(Equation~\ref{eq-gamminteraction}). In the legend, te(predic,freq)
refers to the predicted effect of the interaction effect. Thus, the
brightness of the coloration denotes the strength of the interaction
effect at the point in the graph. Brighter colors denote longer reaction
times.}
...
predictability and frequency of the GAM model containing the interaction
between frequency and predictability for phrasal vs nonphrasal verbs
(Equation~\ref{eq-gamphrasal}). Brighter colors denote longer reaction
times. The left graph is the predicted effect for phrasal verbs (e.g.,
pick up), the right graph is the predicted effect for non-phrasal verbs
(e.g., walk up).}
...
their ability to learn and utilize abstract knowledge
...
(\citeproc{ref-morgan2024}{2024}) also demonstrated that abstract
...
\item
  ...
  preference for the alphabetical form. The abstract ordering preference
  was calculated using the model from Morgan \& Levy
  (\citeproc{ref-morgan2015}{2015}).
\item
...
predicted probability of the next word given the words that came before
it, so in order to calculate the probability of the first word in the
binomial it must occur after some other token. Following this, the
probability of the alphabetical form, \emph{A and B} is:
...
(\citeproc{ref-morgan2015}{2015}) which are defined in
...
was described above. Our independent variable in the first analysis is
the abstract ordering preference for each binomial (AbsPref). In order
to rule out bigram probabilities as driving the model's preferences, our
...
Finally, our independent variables in the third analysis are the
...
The results for the first analysis are presented in
...
these results can be found in Figure~\ref{fig-exp1m1}.
...
\textbf{ } & \textbf{Estimate} & \textbf{Est.Error} & \textbf{Q2.5} & \textbf{Q97.5} & \textbf{\% Samples > 0}\\
\midrule
\textbf{\cellcolor{gray!6}{Intercept}} & \cellcolor{gray!6}{-1.26} & \cellcolor{gray!6}{0.64} & \cellcolor{gray!6}{-2.59} & \cellcolor{gray!6}{-0.11} & \cellcolor{gray!6}{1.46}\\
\textbf{AbsPref} & 2.45 & 1.28 & 0.20 & 5.11 & 98.51\\
\textbf{\cellcolor{gray!6}{BigramProbs}} & \cellcolor{gray!6}{0.41} & \cellcolor{gray!6}{0.49} & \cellcolor{gray!6}{-0.56} & \cellcolor{gray!6}{1.41} & \cellcolor{gray!6}{57.35}\\
\textbf{AbsPref:BigramProbs} & 0.16 & 0.96 & -1.77 & 2.11 & 80.72\\
\bottomrule
\end{tabular}
\endgroup{}
...
The results of this analysis can be found in Table~\ref{tbl-exp1m2}.
...
checkpoint are presented in Table~\ref{tbl-exp2m1} and visualized in
Figure~\ref{fig-exp2m1}.
...
the effect of AbsPref on LogOdds(AandB) for each checkpoint. The x-axis
corresponds to the number of tokens that the model was trained on at a
given checkpoint. The y-axis corresponds to the coefficient estimate.}
...
present a visualization in Figure~\ref{fig-exp2m2}.

\begin{figure}[htbp]

\caption{\label{fig-exp2m2}Visualization of the effect of each
constraint on the ordering preference at each checkpoint. The x-axis
corresponds to the value of the constraint and the y-axis corresponds to
the model's preference for the alphabetical ordering (in log odds). The
checkpoints are listed in terms of the number of tokens the model was
trained on at that checkpoint (e.g., 10B = trained on 10 billion
tokens).}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{Chapters/LLM Ordering Prefs/Writeup/writeup-llm-ordering-prefs_files/figure-pdf/fig-exp2m2-1.pdf}}
...
powerful relatively quickly, but not which are more culturally central.
...
the Google \emph{n-}grams corpus
(\citeproc{ref-linSyntacticAnnotationsGoogle2012}{Lin et al., 2012}).
...
\textbf{\hspace{1em}\cellcolor{gray!6}{Intercept}} & \cellcolor{gray!6}{0.01} & \cellcolor{gray!6}{0.04} & \cellcolor{gray!6}{-0.06} & \cellcolor{gray!6}{0.08} & \cellcolor{gray!6}{61.12}\\
\textbf{\hspace{1em}OverallFreq} & -0.01 & 0.01 & -0.04 & 0.01 & 14.15\\
\textbf{\hspace{1em}\cellcolor{gray!6}{RelFreq}} & \cellcolor{gray!6}{-0.69} & \cellcolor{gray!6}{0.13} & \cellcolor{gray!6}{-0.93} & \cellcolor{gray!6}{-0.44} & \cellcolor{gray!6}{0.00}\\
\textbf{\hspace{1em}OverallFreq:RelFreq} & -0.10 & 0.03 & -0.16 & -0.04 & 0.00\\
\addlinespace[0.3em]
\multicolumn{6}{l}{\textbf{GPT-2 XL}}\\
\textbf{\hspace{1em}\cellcolor{gray!6}{Intercept}} & \cellcolor{gray!6}{-0.02} & \cellcolor{gray!6}{0.04} & \cellcolor{gray!6}{-0.10} & \cellcolor{gray!6}{0.05} & \cellcolor{gray!6}{29.45}\\
\textbf{\hspace{1em}OverallFreq} & 0.03 & 0.01 & 0.01 & 0.06 & 99.67\\
\textbf{\hspace{1em}\cellcolor{gray!6}{RelFreq}} & \cellcolor{gray!6}{0.06} & \cellcolor{gray!6}{0.13} & \cellcolor{gray!6}{-0.19} & \cellcolor{gray!6}{0.31} & \cellcolor{gray!6}{68.51}\\
\textbf{\hspace{1em}OverallFreq:RelFreq} & 0.06 & 0.03 & 0.00 & 0.12 & 97.75\\
\addlinespace[0.3em]
\multicolumn{6}{l}{\textbf{OLMo-1B}}\\
\textbf{\hspace{1em}\cellcolor{gray!6}{Intercept}} & \cellcolor{gray!6}{-0.01} & \cellcolor{gray!6}{0.04} & \cellcolor{gray!6}{-0.08} & \cellcolor{gray!6}{0.06} & \cellcolor{gray!6}{39.92}\\
\textbf{\hspace{1em}OverallFreq} & 0.00 & 0.01 & -0.02 & 0.03 & 61.90\\
\textbf{\hspace{1em}\cellcolor{gray!6}{RelFreq}} & \cellcolor{gray!6}{-0.57} & \cellcolor{gray!6}{0.13} & \cellcolor{gray!6}{-0.81} & \cellcolor{gray!6}{-0.32} & \cellcolor{gray!6}{0.00}\\
\textbf{\hspace{1em}OverallFreq:RelFreq} & -0.04 & 0.03 & -0.09 & 0.02 & 10.21\\
\addlinespace[0.3em]
\multicolumn{6}{l}{\textbf{OLMo-7B}}\\
\textbf{\hspace{1em}\cellcolor{gray!6}{Intercept}} & \cellcolor{gray!6}{-0.01} & \cellcolor{gray!6}{0.04} & \cellcolor{gray!6}{-0.09} & \cellcolor{gray!6}{0.06} & \cellcolor{gray!6}{39.30}\\
\textbf{\hspace{1em}OverallFreq} & 0.00 & 0.01 & -0.02 & 0.03 & 62.40\\
\textbf{\hspace{1em}\cellcolor{gray!6}{RelFreq}} & \cellcolor{gray!6}{-0.57} & \cellcolor{gray!6}{0.13} & \cellcolor{gray!6}{-0.82} & \cellcolor{gray!6}{-0.32} & \cellcolor{gray!6}{0.00}\\
\textbf{\hspace{1em}OverallFreq:RelFreq} & -0.04 & 0.03 & -0.09 & 0.02 & 10.99\\
\addlinespace[0.3em]
\multicolumn{6}{l}{\textbf{Llama2-7B}}\\
\textbf{\hspace{1em}\cellcolor{gray!6}{Intercept}} & \cellcolor{gray!6}{0.01} & \cellcolor{gray!6}{0.04} & \cellcolor{gray!6}{-0.06} & \cellcolor{gray!6}{0.09} & \cellcolor{gray!6}{63.04}\\
\textbf{\hspace{1em}OverallFreq} & -0.02 & 0.01 & -0.05 & 0.00 & 3.88\\
\textbf{\hspace{1em}\cellcolor{gray!6}{RelFreq}} & \cellcolor{gray!6}{-0.18} & \cellcolor{gray!6}{0.13} & \cellcolor{gray!6}{-0.43} & \cellcolor{gray!6}{0.07} & \cellcolor{gray!6}{8.04}\\
\textbf{\hspace{1em}OverallFreq:RelFreq} & -0.05 & 0.03 & -0.11 & 0.00 & 3.57\\
\bottomrule
\end{tabular}
\endgroup{}
...
\pandocbounded{\includegraphics[keepaspectratio]{Chapters/LLM Storage/Writeup/writeup-llm-storage_files/figure-pdf/fig-modelsall-1.pdf}}
...
\citeproc{ref-tenney2019bertrediscoversclassical}{Tenney et al., 2019}),
...
similarity between the holistic embeddings and the compositional
embeddings for each hidden layer of each model checkpoint. Overall
frequency of the binomial is on the x-axis and log odds cosine is on the
y-axis. A larger value of log odds cosine indicates that the embeddings
for the alphabetical ordering are more similar to the compositional
embeddings than the embeddings for the nonalphabetical ordering are. The
color indicates whether the alphabetical ordering is more frequent (red)
or whether the nonalphabetical ordering is more frequent (blue). The
layer number is indicated along the top of the graph and the checkpoint
number is indicated on the right side of the graph.}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{Chapters/LLM Storage/Writeup/writeup-llm-storage_files/figure-pdf/fig-resultsalllayers-1.pdf}}
...
the opposite pattern than the later layers, with the embeddings of the
more frequent ordering being more similar to the compositional
embeddings. Specifically, for earlier layers (e.g., layer 0), as overall
frequency increases, the log odds cosine value (which indicates the
similarity between the embeddings for the alphabetical ordering and the
compositional embeddings in relation to the embeddings for the
nonalphabetical ordering and the compositional embeddings) grows larger
for the binomials that occur more frequently in the alphabetical
ordering (red) than those that occur more in the nonalphabetical
ordering more frequently (blue). In other words, the embeddings for the
more frequent ordering of the binomial become more similar to the
compositional embeddings as the overall frequency of the binomial
increases. For later layers, this effect either reverses or seems to
disappear depending on how much the model has been trained (indicated by
the slopes of the lines becoming less steep, or reversing in direction).
...
example, Ethayarajh
(\citeproc{ref-ethayarajh2019howcontextualare}{2019}) found that any two
...
demonstrated that the dative alternation in English (e.g., \emph{give}
\emph{the ball to him} vs \emph{give him the ball}) also shows evidence
of frequency-dependent preference extremity. Specifically, they
...
conditions. Additionally, Albert Felty et al.
(\citeproc{ref-feltyMisperceptionsSpokenWords}{2013}) demonstrated that
...
\item
  ...
  included in Figure~\ref{fig-corpusplot1}.
\item
...
\includegraphics[width=0.7\linewidth,height=\textheight,keepaspectratio]{Chapters/frequency-dependent preference extremity/frequency-dependent-preference-extremity_files/figure-pdf/fig-noNoisePlot-1.pdf}
...
items may lose part of their internal structure. In Chapter 4, we
...
contributions to theories of language learning and language processing
and then examine some of the interesting questions that come out of this
work.
...
(\citeproc{ref-saffranStatisticalLearning8MonthOld1996}{Saffran et al.,
1996}). This naturally results in high-predictability phrases being
segmented together.

Additionally, holistic storage sheds light on other learning phenomena.
For example, Harmon \& Kapatsinski
(\citeproc{ref-harmonPuttingOldTools2017}{2017}) examined what factors
...
(\citeproc{ref-harmonPuttingOldTools2017}{2017}). Specifically, in
...
Harmon \& Kapatsinski (\citeproc{ref-harmonPuttingOldTools2017}{2017})
...
in \citeproc{ref-misraLanguageModelsLearn2024}{Misra \& Mahowald, 2024})
in future work.
...
multi-word storage (c.f., fragment grammar theories;
\citeproc{ref-morganFormalizingConstructionGrammar}{Morgan et al.,
2023}; \citeproc{ref-odonnellFragmentGrammarsExploring2009}{O'Donnell et
al., 2009}). However, positing multi-word storage generates new
interesting questions for the processing literature. For example,
storing the phrase \emph{I don't know} holistically does not preclude
the learner from generating the phrase compositionally. In fact, the
learner is almost certainly still capable of generating the phrase
compositionally. Thus, in what cases do humans access the holistic
representation instead of generating it compositionally? Further, does
this vary as a function of task (e.g., production vs perception)? For
example, it seems more likely that someone would access the phrase
holistically in production than perception, because in production the
speaker has a specific meaning they wish to convey. However, in
perception it is less clear. For example, when listening to a sentence,
the listener must necessarily listen in a temporally linear manner
(i.e., word-by-word). As such, at what point does the listener access a
holistic representation as opposed to accessing the representation of
the individual parts? Despite the difficulty of this question, it is an
...
other lexical items (\citeproc{ref-mcclelland1984}{McClelland et al.,
1984b}), then positing the holistic storage of phrases suggest that they
...
\bibitem[\citeproctext]{ref-goldbergConstructionGrammar2010}
...
\bibitem[\citeproctext]{ref-juhasz2006binocularcoordinationeyes}
...
\bibitem[\citeproctext]{ref-mcclelland1984}
McClelland, J. L., Elman, J. L., \& LANGUAGE, C. U. S. D. L. J. C. F. R.
...
\bibitem[\citeproctext]{ref-pinker1991rules}
Pinker, S. (1991). Rules of language. \emph{Science}, \emph{253}(5019),
530--535.
...
\bibitem[\citeproctext]{ref-rumelhart1986learningtensesenglish}
