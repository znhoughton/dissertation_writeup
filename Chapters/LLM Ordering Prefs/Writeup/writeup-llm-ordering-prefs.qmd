...
#### Abstract Ordering Preferences Corpus {#sec-abstract-ordering-preferences-corpus}
...
For all three analyses, the dependent variable is LogOdds(AandB), which was described above. Our independent variable in the first analysis is the abstract ordering preference for each binomial (AbsPref). In order to rule out bigram probabilities as driving the model's preferences, our second analysis contains the binomial's bigram probabilities (the odds ratio of product of bigram probabilities of the alphabetical order to the product of bigram probability of the nonalphabetical order) as well. Finally, our independent variables in the third analysis are the individual constraints that are used to calculate AbsPref. The model equations are below in @eq-m1, @eq-m2, and @eq-m3. Note that Formal Markedness and Iconicity were dropped from the second model because the constraint values were zero for all of the binomials. Further, our constraints demonstrated a level of co-linearity. Co-linearity can result in poor model estimates and inflated credible intervals. In order to deal with this, we dropped the constraint with the highest variance inflation factor (which turned out to be the lapse constraint). All other constraints had a variance inflation factor value below 1.5. We then performed backward model selection and dropped the predictors whose credible intervals were most centered around zero until the remaining predictors' credible intervals were all at least 75% greater than or less than zero. This resulted in dropping the no final stress, intense, and percept constraints. We acknowledge that this approach is quite exploratory and thus interpretations at the level of the individual constraint must be taken with a grain of salt.
...
The results for the first analysis are presented in @tbl-exp1m1. Our results suggest that there is a main-effect of abstract ordering preference for OLMo's 7B model. A visualization of these results can be found in @fig-exp1m1.
...
gen_pref_data = gen_pref_data %>%
  mutate(probs = exp(log_odds) / (1 + exp(log_odds)))
...
main_model_plot = ggplot(data = gen_pref_data, aes(x=GenPref, y = log_odds)) +
  ...
  ylab('LogOdds of AandB') +
  xlab('Abstract Ordering Preferences') +
  #xlim(c(0,1)) +
...
While these results suggest that the large language models' ordering preferences are sensitive abstract ordering preferences and not bigram probabilities, it's unclear whether their behavior is similar to humans on the level of the individual constraints. Thus, in the third analysis we examined which specific constraints the model is sensitive to, and to what extent.[^writeup-llm-ordering-prefs-4] For this analysis, following @houghtonTaskdependentConsequencesDisfluency2024, we also present the percentage of posterior samples greater than zero. The results of this analysis can be found in @tbl-exp1m2.

[^writeup-llm-ordering-prefs-4]: It's also important to consider how many of our binomials the constraint even applied to (i.e., how many binomials were the constraints non-zero). For the Culture constraint, 62 of our 131 binomials had a non-zero value. For the Power constraint, 54 were non-zero, for the Frequency constraint, all 131 binomials were non-zero, and for the Len constraint, 85 were.
...
Our model estimates for the effect of AbsPref on LogOdds(AandB) at each checkpoint are presented in @tbl-exp2m1 and visualized in @fig-exp2m1.
...
#| label: fig-exp2m1
#| fig-cap: "Visualization of the model predictions for the effect of AbsPref on LogOdds(AandB) for each checkpoint. The x-axis corresponds to the number of tokens that the model was trained on at a given checkpoint. The y-axis corresponds to the coefficient estimate."
...
Similar to Experiment 2, in our second analysis we present a breakdown of the effects of each individual constraint. In this analysis, however, we demonstrate the effect of each constraint at each checkpoint. The full table results can be found in the the appendix section (@sec-individual-constraints-at-each-checkpoint), but we present a visualization in @fig-exp2m2.

```{r echo = F, fig.width=8, fig.height=9.5, fig.align = 'center', warning = F, message = F}
#| label: fig-exp2m2
#| fig-cap: "Visualization of the effect of each constraint on the ordering preference at each checkpoint. The x-axis corresponds to the value of the constraint and the y-axis corresponds to the model's preference for the alphabetical ordering (in log odds). The checkpoints are listed in terms of the number of tokens the model was trained on at that checkpoint  (e.g., 10B = trained on 10 billion tokens)."
